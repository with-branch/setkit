...
import os
import csv
from setkit.datasets.base.dataset import RootflowDataset, RootflowDataItem
...
class ExampleTextCorpus(RootflowDataset):
    """An example rootflow dataset for unlabeled text data.

    Inherits from :class:`RootflowDataset`.
    Each data item is "Hello there, my index is (odd/even)", depending on the index
    The targets are None.
    """
    ...
    def prepare_data(self, path: str):
        ...
        data = []
        for item in csv_data[1:]:
            data.append(RootflowDataItem(item[1], id=item[0]))
        return data
