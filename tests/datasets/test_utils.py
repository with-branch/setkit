...
def test_batch():
    ...
    for _batch in batch(my_list, batch_size=9):
        assert (len(batch) == 9) or (len(batch) == 1)
    for _batch in batch(my_list, batch_size=7):
        assert (len(batch) == 7) or (len(batch) == 2)


def test_batch_enumerate():
    my_list = [i for i in range(100)]
    for _slice, _batch in batch(my_list, batch_size=5):
        assert _slice == slice(0, 5)
        assert _batch == [0, 1, 2, 3, 4]
        break
    for idx, (_slice, _batch) in enumerate(batch(my_list, batch_size=9)):
        assert _slice.start == idx * 9
        assert _slice.stop == min((idx + 1) * 9, 100)
        assert (len(batch) == 9) or (len(batch) == 1)
    for idx, (_slice, _batch) in enumerate(batch(my_list, batch_size=7)):
...
def test_map_functions():
    ...
    assert map_functions(5, functions) == 4.5
    assert map_functions(13, functions) == 24.5
